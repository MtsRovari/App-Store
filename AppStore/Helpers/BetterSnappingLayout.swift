...
//  AppStore
//
//  Created by Mateus Rovari on 16/11/19.
...
class BetterSnappingLayout: UICollectionViewFlowLayout {
    ...
    override func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint, withScrollingVelocity velocity: CGPoint) -> CGPoint {
        ...
        if proposedContentOffset.x <= 0 ||
            ...
            nextX = collectionView.contentOffset.x + (velocity.x > 0 ?
                collectionView.bounds.size.width : -collectionView.bounds.size.width)
        }
        
        let targetRec = CGRect(x: nextX, y: 0, width:
            collectionView.bounds.size.width, height:
        ...
        let layoutAttributesArray =
            super.layoutAttributesForElements(in: targetRec)
